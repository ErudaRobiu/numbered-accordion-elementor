...
/**
 * Registers assets and the Mega Header widget.
 */
final class Header_Module extends Elementor_Module {

	const STYLE_HANDLE  = 'ehdr-mega-header';
	const SCRIPT_HANDLE = 'ehdr-mega-header';
	...
	public static function description() {
		return esc_html__( 'Adds a "Mega Header" widget: a sticky navigation bar that frosts as you scroll, with mega menu panels and a phone drawer.', 'numbered-accordion' );
	}
	...
	public function register_styles() {
		wp_register_style(
			self::STYLE_HANDLE,
			ERUDA_URL . 'modules/header/assets/css/mega-header.css',
			array(),
			ERUDA_VERSION
		);
	}
	...
	public function register_scripts() {
		wp_register_script(
			self::SCRIPT_HANDLE,
			ERUDA_URL . 'modules/header/assets/js/mega-header.js',
			array(),
			ERUDA_VERSION,
			true
		);
	...
	public function register_widgets( $widgets_manager ) {
		try {
			...
			if ( class_exists( '\ErudaToolkit\Modules\Header\Widgets\Mega_Header_Widget' ) ) {
				$widgets_manager->register( new Widgets\Mega_Header_Widget() );
			}
		} catch ( \Throwable $e ) {
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
				error_log( 'Eruda Toolkit: failed to register the mega header widget - ' . $e->getMessage() );
			}
		}
	}
}
